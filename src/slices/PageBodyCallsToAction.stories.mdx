...
Large images with text placed over the images and optional links. Multiple calls
to action can be displayed which are evenly displayed in a row on desktop, or in
a column on mobile.
...
Each item represents a call to action with a background image, text, and
optional button.
...
slice's width. If a background image is not provided to a call to action, a dark
gray gradient is used.
